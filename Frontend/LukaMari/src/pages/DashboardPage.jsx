  ...
  return (
    <div className="p-6 lg:p-8 max-w-5xl mx-auto">
      ...
      <div className="flex items-start justify-between mb-8">
        <div>
          <h1 className="text-3xl font-bold text-white">
            Welcome back, <span className="text-cyan-400">{profile?.username ?? 'User'}</span>
          </h1>
          <p className="text-gray-400 mt-1">Send and receive secret messages securely.</p>
        </div>
        <button
          onClick={() => navigate('/new-secret')}
          className="flex items-center gap-2 bg-cyan-600 hover:bg-cyan-500 text-white px-5 py-2.5 rounded-xl
                     text-sm font-semibold transition-all shadow-lg shadow-cyan-600/30"
        >
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        <StatCard label="Inbox"     value={stats.inbox}     color="cyan"  />
        <StatCard label="Sent"      value={stats.sent}      color="cyan"  />
      ...
      <div className="bg-white/5 backdrop-blur-sm border border-white/10 rounded-2xl overflow-hidden">
        <div className="flex items-center justify-between px-6 py-4 border-b border-white/5">
          <h2 className="text-white font-semibold flex items-center gap-2">
            <HiOutlineInbox className="w-5 h-5 text-cyan-400" />
            Recent Conversations
          </h2>
          <button
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      ...
                      {label && (
                        <span className="bg-cyan-600/30 text-cyan-300 text-xs px-2 py-0.5 rounded-full font-semibold">
                          {label}
                        </span>
                      )}
                    </div>
